...
const ms = require('ms');
const { getCallerFile } = require('./utils.js');
...
function YemotRouter (options = {}) {
    ...
    function logger (callId, msg, color = 'blue') {
        if (!(ops.printLog ?? mergedDefaults.common.printLog)) return;
        ...
        try {
            ...
            if (error instanceof HangupError) {
                ...
                if (ops.uncaughtErrorHandler) {
                    logger(callId, `💥 Uncaught error. applying uncaughtErrorHandler (${getCallerFile(error).mergedPath})`, 'red');
                    try {
                        await ops.uncaughtErrorHandler(error, call);
                    } catch (err) {
                        if (err instanceof ExitError) {
                            console.log(`👋 the call was exited from the extension /${call.extension} in uncaughtErrorHandler (${getCallerFile(error).mergedPath})` + (error.context ? ` (by ${error.context?.caller} to ${error.context?.target})` : ''));
                        } else {
                    ...
                    logger(callId, `💥 Uncaught error. no uncaughtErrorHandler, throwing error (${getCallerFile(error).mergedPath})`, 'red');
                    throw error;
                }
            }
        }
    ...
    return new Proxy(Router(options), {
        get (target, key) {
            ...
            } else if (['use', 'handle', 'set', 'name', 'length'].includes(key)) {
                return target[key];
            } else {
                throw new Error(`YemotRouter: ${key.toString()} is not supported yet`);
            }
        },
